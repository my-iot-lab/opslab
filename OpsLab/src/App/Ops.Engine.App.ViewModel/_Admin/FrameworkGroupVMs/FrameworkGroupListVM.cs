...
public class FrameworkGroupListVM : BasePagedListVM<FrameworkGroup_View, FrameworkGroupSearcher>
{
    ...
    protected override List<GridAction> InitGridAction()
    {
        if (ConfigInfo.HasMainHost && Wtm.LoginUserInfo?.CurrentTenant == null)
        {
            ...
            {
                this.MakeAction("FrameworkGroup","DataFunction",Localizer["_Admin.DataPrivilege"],Localizer["_Admin.DataPrivilege"],
                        GridActionParameterTypesEnum.SingleId, "_Admin", 800, null, null, x => x.GroupCode)
                .SetShowInRow(),
            };
        }
        else
    ...
    protected override IEnumerable<IGridColumn<FrameworkGroup_View>> InitGridHeader()
    {
        return new List<GridColumn<FrameworkGroup_View>>{
            ...
            {
                string rv = "";
                if(!string.IsNullOrEmpty(a.ManagerName))
                {
                    rv += a.ManagerName;
                }
                if(!string.IsNullOrEmpty(a.Manager))
                {
                    rv += $"({a.Manager})";
                }
                return rv;
    ...
    public override IOrderedQueryable<FrameworkGroup_View> GetSearchQuery()
    {
        return DC.Set<FrameworkGroup>()
            ...
            .GroupJoin(DC.Set<FrameworkUser>(), ok => ok.Manager, ik => ik.ITCode, (group, user) => new { user, group })
