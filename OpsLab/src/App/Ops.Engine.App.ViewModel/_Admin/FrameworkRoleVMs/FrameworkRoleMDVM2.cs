...
public class FrameworkRoleMDVM2 : BaseCRUDVM<FrameworkRole>
{
    ...
    protected override void InitVM()
    {
        var allowedids = DC.Set<FunctionPrivilege>()
                                    .Where(x => x.RoleCode == Entity.RoleCode && x.Allowed == true)
                                    .Select(x => x.MenuItemId)
                                    .ToList();
        List<FrameworkMenu> data;
        using (var maindc = Wtm.CreateDC(false, "default"))
        {
            data = maindc.Set<FrameworkMenu>().ToList();
        }

        var topdata = data.Where(x => x.ParentId == null)
                            .ToList()
                            .FlatTree(x => x.DisplayOrder)
                            .Where(x => x.IsInside == false || x.FolderOnly || string.IsNullOrEmpty(x.MethodName))
                            .ToList();

        if (Wtm.ConfigInfo.EnableTenant && LoginUserInfo.CurrentTenant != null)
        {
            var ct = Wtm.GlobaInfo.AllTenant.Where(x => x.TCode == LoginUserInfo.CurrentTenant).FirstOrDefault();
            for (int i = 0; i < topdata.Count; i++)
            {
                if (topdata[i].TenantAllowed == false || 
                    (topdata[i].Url != null 
                        && !ct.EnableSub 
                        && topdata[i].Url.ToLower().Contains("frameworktenant")))
                {
                    topdata.RemoveAt(i);
                    i--;
                    continue;
        ...
        var data2 = topdata.Select(x => new Page_View
        {
            ID = x.ID,
            Name = x.PageName,
            AllActions = x.FolderOnly ? null : x.Children.ToListItems(y => y.ActionName, y => y.ID, null),
            ParentID = x.ParentId,
            Level = x.GetLevel(),
    ...
    public async Task<bool> DoChangeAsync()
    {
        ...
        {
            FunctionPrivilege fp = new() { ID = oldid };
