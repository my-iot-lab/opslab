...
public partial class FrameworkTenantVM : BaseCRUDVM<FrameworkTenant>
{
    ...
    {
        List<FunctionPrivilege> fps = new();
        using (var userdc = LoginUserInfo.GetUserDC(Wtm))
        {
            ...
            List<Guid> tenantmenus = new();
            using (var defaultdc = Wtm.CreateDC(false, "default", false))
            {
                tenantmenus = defaultdc.Set<FrameworkMenu>()
                    .Where(x => x.TenantAllowed == false || (x.Url.ToLower().Contains("frameworktenant") && !Entity.EnableSub) || x.Url.ToLower().Contains("frameworkmenu"))
        ...
        if (Entity.IsUsingDB)
        {
            try
            {
                ...
                if (tenantdc.Database.EnsureCreated())
                {
    ...
    {
        var itcode = "admin";
        var role = dc.Set<FrameworkRole>().AsNoTracking().IgnoreQueryFilters().FirstOrDefault(x => x.RoleCode == "001" && x.TenantCode == Entity.TCode);
        if (role == null)
        {
        ...
        var admin = dc.Set<FrameworkUser>().AsNoTracking().IgnoreQueryFilters().FirstOrDefault(x => x.ITCode == itcode && x.TenantCode == Entity.TCode);
        if (admin == null)
        {
        ...
        var middle = dc.Set<FrameworkUserRole>()
            .AsNoTracking()
            .IgnoreQueryFilters()
            .FirstOrDefault(x => x.UserCode == "admin" && x.RoleCode == "001" && x.TenantCode == Entity.TCode);
        if (middle == null)
        {
