...
public class ChangePasswordVM : BaseVM
{
    ...
    {
        List<ValidationResult> rv = new();
        if (DC.Set<FrameworkUser>().SingleOrDefault(x => x.ITCode == LoginUserInfo.ITCode && x.Password == Utils.GetMD5String(OldPassword)) == null)
        {
    ...
    {
        var user = DC.Set<FrameworkUser>().SingleOrDefault(x => x.ITCode == LoginUserInfo.ITCode);
        if (user != null)
        {
