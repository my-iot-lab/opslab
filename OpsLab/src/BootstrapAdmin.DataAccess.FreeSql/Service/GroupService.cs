...
class GroupService : IGroup
{
    ...
    {
        return FreeSql.Ado.Query<string>("select GroupID from RoleGroup where RoleID = @roleId", new { roleId });
    });
    ...
    {
        return FreeSql.Ado.Query<string>("select GroupID from UserGroup where UserID = @userId", new { userId });
    });
