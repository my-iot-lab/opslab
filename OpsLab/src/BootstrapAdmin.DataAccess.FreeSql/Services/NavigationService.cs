...
class NavigationService : INavigation
{
    ...
    {
        return FreeSql.Ado.Query<Navigation>(@"
select n.ID, n.ParentId, n.Name, n.Order, n.Icon, n.Url, n.Category, n.Target, n.IsResource, n.Application, ln.Name as ParentName 
from Navigations n 
    inner join Dicts d on n.Category = d.Code and d.Category = @Category 
    left join Navigations ln on n.ParentId = ln.ID 
inner join (select nr.NavigationID 
...
ORDER BY n.Application, n.Order", new { UserName = userName, Category = "菜单", RoleName = "Administrators" });
    });

    public List<string> GetMenusByRoleId(string? roleId) => 
        CacheManager.GetOrAdd($"{NavigationServiceGetMenusByRoleIdCacheKey}-{roleId}", entry => 
            FreeSql.Ado.Query<string>("select NavigationID from NavigationRole where RoleID = @roleId", new { roleId }));
