...
/// 本类自动启动一个静态线程来处理
/// </remarks>
public sealed class ConnTimeOut
{
	private static long s_opsTimeoutId = 0L;
	private static readonly List<ConnTimeOut> WaitHandleTimeOut = new(128);
	...
	/// <summary>
	/// 新增一个超时检测的对象，当操作完成的时候，需要自行标记<see cref="ConnTimeOut" />对象的<see cref="IsSuccessful" />为<c>True</c>
	/// </summary>
	/// <param name="timeOut">超时对象</param>
	public static void HandleTimeOutCheck(ConnTimeOut timeOut)
	{
		lock (ListLock)
		{
	...
	public static ConnTimeOut[] GetHslTimeOutsSnapShoot()
	{
		lock (ListLock)
		{
			return [.. WaitHandleTimeOut];
		}
	}

	/// <summary>
	/// 新增一个超时检测的对象，需要指定socket，超时时间，返回<see cref="ConnTimeOut" />对象，用作标记完成信息
	/// </summary>
	/// <param name="socket">网络套接字</param>
	/// <param name="timeout">超时时间，单位为毫秒</param>
	public static ConnTimeOut HandleTimeOutCheck(Socket socket, int timeout)
	{
		var hslTimeOut = new ConnTimeOut
		{
			DelayTime = timeout,
			IsSuccessful = false,
			StartTime = DateTime.Now,
	...
	private static void CheckTimeOut(object obj)
	{
		...
		while (true)
		{
			...
			lock (ListLock)
			{
				...
				{
					ConnTimeOut opsTimeOut = WaitHandleTimeOut[num2];
