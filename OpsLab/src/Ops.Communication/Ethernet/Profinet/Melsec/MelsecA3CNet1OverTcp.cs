...
public class MelsecA3CNet1OverTcp : NetworkDeviceBase
{
	...
	public async Task<OperateResult<string>> ReadPlcTypeAsync()
	{
		...
		return OperateResult.CreateSuccessResult(Encoding.ASCII.GetString(read.Content, 11, 16).TrimEnd(Array.Empty<char>()));
	}
	...
	public static OperateResult<string> ReadPlcTypeHelper(byte station, Func<byte[], byte, OperateResult<byte[]>> readCore)
	{
		...
		return OperateResult.CreateSuccessResult(Encoding.ASCII.GetString(operateResult.Content, 11, 16).TrimEnd(Array.Empty<char>()));
	}
