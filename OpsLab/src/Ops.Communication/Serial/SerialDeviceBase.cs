...
/// </list>
/// </remarks>
public abstract class SerialDeviceBase : SerialBase, IReadWriteDevice, IReadWriteNet
{
	...
	{
		var result = await ReadAsync(address, (ushort)(length * WordLength)).ConfigureAwait(false);
		return ByteTransformHelper.GetResultFromBytes(result, (byte[] m) => ByteTransform.TransInt16(m, 0, length));
	}
	...
	{
		var result = await ReadAsync(address, (ushort)(length * WordLength * 2)).ConfigureAwait(false);
		return ByteTransformHelper.GetResultFromBytes(result, (byte[] m) => ByteTransform.TransInt32(m, 0, length));
	}
	...
	{
		var result = await ReadAsync(address, (ushort)(length * WordLength * 4)).ConfigureAwait(false);
		return ByteTransformHelper.GetResultFromBytes(result, (byte[] m) => ByteTransform.TransInt64(m, 0, length));
	}
	...
	{
		var result = await ReadAsync(address, length).ConfigureAwait(false);
		return ByteTransformHelper.GetResultFromBytes(result, (byte[] m) => ByteTransform.TransString(m, 0, m.Length, encoding));
	}
